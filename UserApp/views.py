...
from .ytDownloader import downloadSong
...
def PlaySong(request, sid):
    ...
    if("uname" in request.session):
        try:
            fav_song = Favourite.objects.get(
                user_id=request.session["uname"], song_id=sid)
            flag = "True"
        except:
            flag = "False"
...
def downloadMp3(request):
    if request.method == "POST":
        ...
        song = downloadSong(link)
        song = str(song)
        return render(request, 'UserApp/downloadYtube.html', {"song": song})
    else:
        return render(request, 'UserApp/downloadYtube.html', )
